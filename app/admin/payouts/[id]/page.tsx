...
export default function AdminPayoutDetailsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-10">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-widest text-gray-400">Payout detail</p><h1 className="mt-1 break-all text-2xl font-black text-gray-900">{payout.id}</h1><p className="mt-1 text-sm text-gray-500">Requested {dateValue(payout.requestedAt)}</p></div><StatusBadge status={status} size="md" /></div>
      <div className="grid gap-4 md:grid-cols-3"><Info icon={<DollarSign size={16} />} label="Gross amount" value={money(payout.grossAmount || payout.amount)} /><Info icon={<DollarSign size={16} />} label="Platform fee" value={money(payout.platformFee)} /><Info icon={<DollarSign size={16} />} label="Net payout" value={money(payout.netAmount)} /></div>
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm"><h2 className="mb-4 flex items-center gap-2 font-black"><FileText size={17} className="text-green-600" /> Provider and seller</h2><Info label="Seller" value={text(payout.vendorId || payout.storeId)} /><Info label="Bank" value={`${text(payout.bankName)} · ${text(payout.accountNumber)}`} /><Info label="Account name" value={text(payout.accountName)} /><Info label="Nomba reference" value={text(payout.nombaReference || payout.reference)} /><Info label="Provider reference" value={text(payout.providerReference || payout.nombaReference || payout.reference)} /><Info label="Provider status" value={text(payout.providerStatus)} /></div>
        <div className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm"><h2 className="mb-4 flex items-center gap-2 font-black"><Clock3 size={17} className="text-purple-600" /> Reconciliation timeline</h2><Info label="Gateway submitted" value={dateValue(payout.gatewaySubmittedAt)} /><Info label="Completed" value={dateValue(payout.completedAt)} /><Info label="Balance restored" value={dateValue(payout.balanceRestoredAt)} /><Info label="Reconciled" value={dateValue(payout.reconciledAt)} /><Info label="Reason" value={text(payout.reconciliationReason || payout.failureReason || payout.refundReason)} /></div>
      </section>
