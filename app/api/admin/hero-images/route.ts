...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (file.size <= 0 || file.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: "Hero images must be smaller than 8 MB" }, { status: 400 });
    }

    const fileName = `${safeBaseName(file.name)}-${randomUUID().slice(0, 8)}${extension}`;
    const relativePath = path.join("images", "hero", fileName);
    const uploadDirectory = path.join(process.cwd(), "public", "images", "hero");
    ...
    console.error("Admin hero image upload error:", error);
    return NextResponse.json({ error: "Hero image could not be uploaded" }, { status: 500 });
  }
}
