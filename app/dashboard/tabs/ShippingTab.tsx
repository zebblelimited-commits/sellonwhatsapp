...
import { CourierLogo, ShipmentJourney } from "@/components/shipping/ShippingVisuals";
...
type SellerShipment = {
  ...
  createdAt: Date; trackingId?: string; dispatchStatus?: string; dispatchError?: string; items: any[];
};
...
const statusConfig: Record<string, { label: string; className: string; icon: typeof Clock3 }> = {
  ...
  self_arranged: { label: "Self-arranged delivery", className: "bg-emerald-50 text-emerald-700 border-emerald-100", icon: MapPin },
};
      ...
      const publish = () => {
        ...
        orderSources.forEach((rows) => rows.forEach((order) => {
          ...
          merged.set(order.id, {
            ...
            createdAt: dateOf(shipment?.createdAt || order.createdAt), trackingId: shipment?.trackingId || order.trackingId, dispatchStatus: shipment?.dispatchStatus, dispatchError: shipment?.dispatchError,
...
function ShipmentRow({ shipment, onClick }: { shipment: SellerShipment; onClick: () => void }) { const config = statusConfig[shipment.status] || statusConfig.pending_payment; const Icon = config.icon; const dispatchFailed = shipment.dispatchStatus === "FAILED" || shipment.dispatchStatus === "PROVIDER_INTEGRATION_REQUIRED"; return <button onClick={onClick} className="group flex w-full flex-col gap-4 rounded-2xl border border-gray-100 p-4 text-left transition hover:border-emerald-200 hover:shadow-md sm:flex-row sm:items-center"><div className="flex min-w-0 flex-1 items-center gap-3"><CourierLogo courierName={shipment.courierName} size="md" /><div className="min-w-0"><p className="truncate text-sm font-black text-gray-900">{shipment.buyerName}</p><p className="mt-1 truncate text-xs font-medium text-gray-500">Order {shipment.orderId} · {shipment.createdAt.toLocaleDateString("en-NG", { day: "numeric", month: "short" })}</p></div></div><div className="flex items-center justify-between gap-4 sm:justify-end"><div><span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${config.className}`}><Icon size={12} /> {config.label}</span><p className="mt-2 flex items-center gap-1.5 text-xs font-bold text-gray-600"><Truck size={13} className="text-gray-400" /> {shipment.courierName}</p>{dispatchFailed && <p className="mt-1 text-[10px] font-black text-rose-600">Dispatch needs attention</p>}</div><ChevronRight size={18} className="text-gray-300 transition group-hover:translate-x-1 group-hover:text-emerald-600" /></div></button>; }
function Details({ shipment, onClose }: { shipment: SellerShipment; onClose: () => void }) { const config = statusConfig[shipment.status] || statusConfig.pending_payment; const Icon = config.icon; return <div className="fixed inset-0 z-[120] flex items-end justify-center bg-slate-950/40 p-3 backdrop-blur-sm sm:items-center" onClick={onClose}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[28px] bg-white p-6 shadow-2xl sm:p-8" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Shipment details</p><h3 className="mt-1 text-xl font-black text-gray-900">{shipment.buyerName}</h3><p className="mt-1 text-xs font-bold text-gray-400">Order {shipment.orderId}{shipment.buyerPhone ? ` · ${shipment.buyerPhone}` : ""}</p></div><button onClick={onClose} className="rounded-xl px-3 py-2 text-xs font-black text-gray-500 hover:bg-gray-100">Close</button></div><div className="mt-5 flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3"><CourierLogo courierName={shipment.courierName} size="md" /><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-700">Assigned courier</p><p className="mt-1 text-sm font-black text-gray-900">{shipment.courierName}</p></div></div><div className="mt-6 rounded-2xl bg-gray-50 p-4"><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-black uppercase tracking-wide ${config.className}`}><Icon size={13} /> {config.label}</span><div className="mt-5 grid gap-4 sm:grid-cols-2"><Address label="Pickup from your store" value={shipment.pickupAddress} /><Address label="Deliver to buyer" value={shipment.deliveryAddress} /></div><div className="mt-6"><ShipmentJourney status={shipment.status} courierName={shipment.courierName} createdAt={shipment.createdAt} /></div></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><Info label="Courier" value={shipment.courierName} /><Info label="Delivery fee" value={money(shipment.shippingCost)} /><Info label="Items" value={String(shipment.items.length || 1)} /></div>{shipment.trackingId && <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Tracking reference</p><p className="mt-1 break-all text-sm font-black text-indigo-950">{shipment.trackingId}</p></div>}</div></div>; }
...
function Empty() { return <div className="rounded-2xl bg-gray-50 p-10 text-center"><PackageCheck className="mx-auto text-gray-300" size={34} /><p className="mt-3 text-sm font-black text-gray-700">No shipments found</p><p className="mt-1 text-xs font-medium text-gray-400">Courier and self-arranged delivery orders will appear here.</p></div>; }
