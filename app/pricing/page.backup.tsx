...
import {
  ...
  Crown, ChevronDown, Loader2, ToggleLeft, ToggleRight
} from "lucide-react";
...
// ✅ Plan configuration with duration support
interface Plan {
  id: string;
  name: string;
  price: number;
...
// ✅ Price calculator (matches server logic)
...
export default function PricingPage() {
  ...
  // ✅ Handle plan selection (Free vs Paid)
  ...
  // ✅ Handle paid plan checkout via Nomba
  const handlePaidCheckout = async (
    ...
    try {
      ...
      // Calculate prices
      const monthlyBasePrice = plan.price; // e.g., 4999 for Pro Lite
      const totalBasePrice = monthlyBasePrice * duration.months; // Total without discount
      ...
      const response = await fetch("/api/premium/subscription-checkout", {
        ...
        body: JSON.stringify({
          ...
          monthlyBasePrice: monthlyBasePrice,     // Price per month (e.g., 4999)
          basePrice: totalBasePrice,               // Total before discount
          finalPrice: finalPrice,                  // Total after discount (what to charge)
          discount: duration.discount,
      ...
      showNotification("info", "Redirecting to Payment",
        `Complete your payment of ₦${finalPrice.toLocaleString()} for ${plan.name} (${duration.label})`
      );

      // Optional: Log the redirect for debugging
      console.log('Redirecting to Nomba checkout:', data.checkoutLink);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {PLANS.map((plan) => (
              ...
              >
                ...
                {/* ✅ Price Display - White text for dark variant */}
                <div className="mb-8">
                  {!plan.isFree ? (
                    <>
                ...
                {/* ✅ Savings badge - Better contrast for dark variant */}
...
// ✅ Duration Selector Component - FIXED: Dark blue active state for dark variant
          ...
          return (
            <button
              ...
              className={`relative p-3 rounded-xl border-2 text-left transition-all ${
                // ✅ ACTIVE STATE: Dark blue for dark variant, green for light
                isSelected
                  ? isDark
                    ? "border-blue-400 bg-blue-900/60 shadow-lg shadow-blue-500/20"  // ✅ Dark blue active
                    : "border-green-600 bg-green-50"                                   // Original green active
            ...
            >
              ...
              <div className="flex items-center justify-between mb-1">
                <span className={`text-xs font-bold ${
                  // ✅ Label: White when selected on dark, otherwise conditional
                  isSelected && isDark ? "text-white" : isDark ? "text-white" : "text-gray-900"
                  }`}>{option.label}</span>
                {option.discount > 0 && (
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                    // ✅ Discount badge: Blue tones when selected on dark
                    isSelected && isDark
              ...
              <div className="flex items-baseline gap-1">
                <span className={`text-lg font-black ${
                  // ✅ Price: White when selected on dark
                  isSelected && isDark ? "text-white" : isDark ? "text-white" : "text-gray-900"
                  }`}>₦{finalPrice.toLocaleString()}</span>
                {!isYearly && (
                  <span className={`text-[10px] ${
                    // ✅ "total": Light blue when selected on dark for contrast
              ...
              {!isYearly && (
                <p className={`text-[9px] mt-0.5 ${
                  // ✅ "/mo": Light blue when selected on dark
                  isSelected && isDark ? "text-blue-200" : isDark ? "text-gray-300" : "text-gray-400"
                  }`}>
              ...
              {option.discount > 0 && (
                <p className={`text-[9px] font-bold mt-1 ${
                  // ✅ Savings: Light blue when selected on dark
...
// ✅ Auto-Renew Toggle Component - Accepts isDark prop for theming
