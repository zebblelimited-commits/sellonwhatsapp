...
type Profile = { uid?: string; email?: string; displayName?: string; phoneNumber?: string; timezone?: string; role?: string; isActive?: boolean };
...
export default function AdminSettingsPanel() {
  ...
  async function load() {
    ...
    try {
      ...
      if (nextProfile.role === "super_admin") {
        ...
        if (adminsResponse.ok) setAdmins(adminsPayload.admins || []);
      }
  ...
  async function changePassword() {
    ...
    } catch (passwordError) { setError(passwordError instanceof Error ? passwordError.message : "Password could not be changed"); }
  }
  ...
  return <div className="max-w-5xl space-y-6 animate-in fade-in duration-300">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <section className="space-y-4 rounded-[28px] border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <label className="block text-xs font-bold text-gray-500">Timezone<input value={timezone} onChange={(event) => setTimezone(event.target.value)} className="mt-1 w-full rounded-xl border border-gray-200 p-3 text-sm outline-none focus:ring-2 focus:ring-green-500" /></label>
      ...
      <section className="space-y-4 rounded-[28px] border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2"><button onClick={() => void changePassword()} className="rounded-xl bg-gray-900 px-4 py-2 text-sm font-bold text-white">Change password</button><button onClick={() => void resetPassword()} className="rounded-xl bg-gray-100 px-4 py-2 text-sm font-bold text-gray-700">Email reset link</button></div>
