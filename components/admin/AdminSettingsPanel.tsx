...
export default function AdminSettingsPanel() {
  ...
  return <div className="max-w-5xl space-y-6 animate-in fade-in duration-300">
    ...
    <section className="space-y-4 rounded-[28px] border border-gray-100 bg-white p-6 shadow-sm">
      ...
      {couriers.length === 0 && !couriersError ? <div className="rounded-2xl bg-gray-50 p-4 text-sm text-gray-500">Loading courier options...</div> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{couriers.map((courier) => <div key={courier.id} className="flex items-center justify-between gap-3 rounded-2xl border border-gray-100 p-3"><div className="flex min-w-0 items-center gap-3"><div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border border-gray-200 bg-white shadow-sm"><Image src={courier.logo} alt="" fill sizes="40px" className="object-contain p-1.5" /></div><div className="min-w-0"><p className="truncate text-sm font-bold text-gray-900">{courier.name}</p><p className="truncate text-[10px] text-gray-400">{courier.estimatedDays}</p></div></div><button type="button" aria-pressed={courier.isActive} aria-label={`${courier.isActive ? "Disable" : "Enable"} ${courier.name}`} disabled={courierSaving === courier.id} onClick={() => void toggleCourier(courier)} className={`relative h-6 w-11 shrink-0 rounded-full transition-colors disabled:cursor-wait disabled:opacity-60 ${courier.isActive ? "bg-green-600" : "bg-gray-300"}`}><span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform ${courier.isActive ? "translate-x-5" : "translate-x-0.5"}`} /></button></div>)}</div>}
    </section>
