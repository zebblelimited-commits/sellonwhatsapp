...
function ShipmentRow({ record, onClick }: { record: ShippingRecord; onClick: () => void }) {
  ...
  return <button onClick={onClick} className="group flex w-full flex-col gap-4 rounded-2xl border border-gray-100 p-4 text-left transition hover:border-emerald-200 hover:shadow-md sm:flex-row sm:items-center"><div className="flex min-w-0 flex-1 items-center gap-3"><CourierLogo courierId={record.courierId} courierName={record.courierName} size="md" /><div className="min-w-0"><p className="truncate text-sm font-black text-gray-900">{record.storeName}</p><p className="mt-1 truncate text-xs font-medium text-gray-500">Order {record.orderId} · {formatDate(record.createdAt)}</p></div></div><div className="flex items-center justify-between gap-4 sm:justify-end"><div><span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${meta.tone}`}><Icon size={12} /> {meta.label}</span><p className="mt-2 flex items-center gap-1.5 text-xs font-bold text-gray-600"><Truck size={13} className="text-gray-400" /> {record.courierName}</p></div><ChevronRight size={18} className="text-gray-300 transition group-hover:translate-x-1 group-hover:text-emerald-600" /></div></button>;
}

function ShipmentDetails({ record, onClose }: { record: ShippingRecord; onClose: () => void }) {
  const meta = statusForDisplay(record); const Icon = meta.icon;
  return <div className="fixed inset-0 z-[120] flex items-end justify-center bg-slate-950/40 p-3 backdrop-blur-sm sm:items-center" onClick={onClose}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[28px] bg-white p-6 shadow-2xl sm:p-8" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Shipment details</p><h3 className="mt-1 text-xl font-black text-gray-900">{record.storeName}</h3><p className="mt-1 text-xs font-bold text-gray-400">Order {record.orderId}</p></div><button onClick={onClose} className="rounded-xl px-3 py-2 text-xs font-black text-gray-500 hover:bg-gray-100">Close</button></div><div className="mt-5 flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3"><CourierLogo courierId={record.courierId} courierName={record.courierName} size="md" /><div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-700">Assigned courier</p><p className="mt-1 text-sm font-black text-gray-900">{record.courierName}</p></div></div><div className="mt-6 rounded-2xl bg-gray-50 p-4"><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-black uppercase tracking-wide ${meta.tone}`}><Icon size={13} /> {meta.label}</span><div className="mt-5 grid gap-4 sm:grid-cols-2"><Address label="Pickup from seller" value={record.pickupAddress} /><Address label="Deliver to you" value={addressText(record.deliveryAddress)} /></div><div className="mt-6"><ShipmentJourney status={record.orderStatus || record.status} courierId={record.courierId} courierName={record.courierName} createdAt={record.createdAt} /></div></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><Info label="Courier" value={record.courierName} /><Info label="Estimated time" value={record.estimatedDays || "Provided by courier"} /><Info label="Delivery fee" value={formatCurrency(record.shippingCost)} /></div>{record.trackingId && <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Tracking reference</p><p className="mt-1 break-all text-sm font-black text-indigo-950">{record.trackingId}</p></div>}</div></div>;
}
